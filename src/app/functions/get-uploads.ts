...
export async function getUploads(
  ...
  const { pageSize, page, searchQuery, sortBy, sortDirection } =
    getUploadsInput.parse(input)

  const [uploads, [{ total }]] = await Promise.all([
    // Primeira query com todos os valores de filtragem
    db
      .select({
    ...
    // Segunda query com valores apenas para o total de paginas
    db
